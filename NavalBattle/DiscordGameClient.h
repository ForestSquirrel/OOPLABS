...
public:
    ...
    SleepyDiscord::Embed embedTemp, embedHelp;
    SleepyDiscord::EmbedField fieldTemp, fieldHelp;
    bool Turn = true; //true - first player, false - second player

    void onMessage(SleepyDiscord::Message message) {
        //
        if (message.startsWith("NB do")) {
            sendMessage(message.channelID, " dfsdfsf");
        }
        ...
        if (message.startsWith("NB reg")) {
            if (GH.getFirstPlayerName().empty()) {
        ...
        if (message.startsWith("NB shoot")) {
            ...
            std::string tile;
            std::string msg;
            for (auto i = 10; i < message.content.size(); i++) {    // i = 10 because starts with ...
            ...
            if (message.author.username == GH.getFirstPlayerName() && Turn) {
                try {
                    ...
                    embedTemp.fields.clear();
                    embedTemp.title = "Your opponents field";
                    fieldTemp.isInline = true;
                    fieldTemp.name = "Your shot was " + message.content;
                    fieldTemp.value = ASCIIartField(GH.getBlankFieldByPlayerMatrix(GH.getFirstPlayerName(), true));
                ...
                try {
                    ...
                    fieldTemp.isInline = true;
                    fieldTemp.name = "Your shot was " + message.content;
                    fieldTemp.value = ASCIIartField(GH.getBlankFieldByPlayerMatrix(GH.getSecondPlayerName(), true));
                    embedTemp.fields.push_back(fieldTemp);
        ...
        if (message.startsWith("NB show me")) {
            ...
            if (message.author.username == GH.getFirstPlayerName()) {
                try {
                    embedTemp.fields.clear();
                    embedTemp.title = "My field";
                    fieldTemp.isInline = true;
                    fieldTemp.name = "Your field";
                    fieldTemp.value = ASCIIartField(GH.getBlankFieldByPlayerMatrix(GH.getFirstPlayerName(), false));
                    embedTemp.fields.push_back(fieldTemp);
                    sendMessage(message.channelID, msg, embedTemp);
                }
                catch (std::invalid_argument& e) {
                ...
                try {
                    ...
                    fieldTemp.isInline = true;
                    fieldTemp.name = "Your field";
                    fieldTemp.value = ASCIIartField(GH.getBlankFieldByPlayerMatrix(GH.getSecondPlayerName(), false));
                    embedTemp.fields.push_back(fieldTemp);
                    sendMessage(message.channelID, msg, embedTemp);
                }
                catch (std::invalid_argument& e) {
